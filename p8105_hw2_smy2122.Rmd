...
Each dataset describes trash collected by different trash wheels around Baltimore, Maryland. The first dataset in this problem, Mr. Trash Wheel, contains `r nrow(mr_trash_wheel_df)` rows and `r ncol(mr_trash_wheel_df)` columns. The second dataset in this problem, Professor Trash Wheel, contains `r nrow(prof_trash_wheel_df)` rows and `r ncol(prof_trash_wheel_df)` columns. The third dataset in this problem, Gwynnda Trash Wheel, contains `r nrow(gwynnda_trash_wheel_df)` rows and `r ncol(gwynnda_trash_wheel_df)` columns. Each row represents one different dumpster that the trash wheel picks up trash from; thus, Mr. Trash Wheel collects trash from the most dumpsters, followed by Gwynnda Trash Wheel and Professor Trash Wheel. The columns for each dataset tell us the `date` of trash collection, `weight` (in tons) and `volume` (in cubic yards) of that dumpster's trash, and different types of trash that the trash wheels have collected, such as `plastic bottles` and `grocery bags`.
